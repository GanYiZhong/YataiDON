...
CostumeMenu::CostumeMenu(PlayerNum player_num) : player_num(player_num), is_2p(player_num == PlayerNum::P2) {
    ...
    presets_enabled = tex.skin_flag("costume_preset_slots");
...
// The wardrobe (Models/costume_icon, ids as filenames) plus, optionally, the
// per-preset membership sets. The cabinet's own sets live in datatable/don_cos.bin
// keyed by arcade part ids; our Models are a repacked 0..N id space, so that table
// cannot be ported. A skin may therefore ship its own curation in
// Models/costume_presets.json:
//     { "default": [0], "cool": [...], "cute": [...], "joke": [...], "random": [] }
// A key that is absent (or an empty list) means "the whole wardrobe", which is
// exactly what don_cos.bin's `choice` column is: true for every head, body and face.
// "default" additionally falls back to id 0 — the plain Don, kPreset_Default.
...
// Arcade Don.ChangePresetDon(player, preset): dress the Don in that preset now, while
// the cursor is on it. RANDOM_ITEM re-rolls on every landing; the curated presets keep
// the id they drew for the lifetime of this menu, the way the cabinet's prebuilt row
// stays put inside the screen.
...
std::string CostumeMenu::get_costume_name() const {
    // A highlighted preset outranks the (unused) sub-list while we are on the top row.
...
void CostumeMenu::handle_input() {
    if (costume_select_mode) {
        ...
        // Arcade UpdateDon(): the cursor landing on a row IS the costume change.
        if (presets_enabled && selected_index != prev_index) {
            if (is_preset_item(ITEMS[selected_index])) {
                ...
                // The two YataiDON-only rows are not presets; drop the preview so the
                // Don goes back to what the save data says he is wearing.
                preset_cos_id.reset();
                preset_seq++;
            }
        }
        ...
        if (is_l_don_pressed(player_num) || is_r_don_pressed(player_num)) {
            if (ITEMS[selected_index] == COSTUME_SELECT::COSTUME) {
                ...
                // Arcade PlayDataManager.DecideCostumeSelect(player, preset): commit the
                // outfit that is on the Don right now. There is no cancel on this screen.
...
void CostumeMenu::draw(float x, float y) {
    // preset_seq is a 5th argument: skins that don't take it (PyTaikoGreen) ignore it,
    // the way Lua ignores any extra argument. It bumps on every preset roll, which is
    // the frame the arcade fires smoke_instance "smoke_in".
