...
private:
    ...
    sol::protected_function fn_decide_side_select;
...
public:
    ...
    // Optional hook (ROUND 12): the engine has no CreditDecideAction state, so it names
    // the row the arcade would flash at the moment the seat enters. Skins may omit it.
    void decide_side_select(int side);
