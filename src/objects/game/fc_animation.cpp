...
FCAnimation::FCAnimation(bool is_2p, bool donderful)
    ...
    // A skin opts into ドンダフルコンボ by shipping a game/ending_donderful subset
    // that mirrors the full-combo banner names; everything else is shared with the
    // regular full-combo animation.
    ...
    // The arcade's ドンダフルコンボ rig (enso/information/action_result.nulm,
    // sprite 223 `dondafullMc`) carries a 1424x264 gold panel at depth 0 -- behind
    // the fans, the lettering and everything else in the clip -- that fades in
    // over 15 frames (250 ms) at the same moment as the banner and then holds.
    // Optional: a skin that ships no `ending_donderful/background` renders exactly
    // as before.
...
void FCAnimation::update(double current_ms) {
    ...
    if (clear_highlight_fade_in->attribute == 1.0f) {
        // The panel's fade starts on the same beat the banner appears, which is
        // the arcade relationship (both run over dondafullMc frames 74..88).
...
void FCAnimation::draw() {
    if (draw_clear_full) {
        // Backing panel first -- depth 0 of the arcade clip, under the fans.
        if (has_panel) {
        ...
        // Fans first: the banner text sits on top of them, as in the arcade.
