...
void Fireworks::draw() {
    if (!explosion_anim->is_finished) {
        // Optional skin key `gogo_explosion_slots {x = slot count, y = first
        // slot drawn mirrored}`: the arcade `gogo_effect` splash group has SIX
        // clips, the right-hand two horizontally mirrored. Without the key the
        // original five un-mirrored slots are drawn.
        int slots = 5;
        int mirror_from = -1;
