...
ScoresManager::ScoresManager(const fs::path& db_path) {
    ...
    // that took them under the upstream numbering fails harmlessly.
    if (version < 4) {
...
void ScoresManager::add_path_binding(const fs::path& path, const std::array<std::string, 5>& hashes) {
    ...
    // ROUND 46 (r46-recentplayed-2p-crash): every chart with no recorded
    // per-difficulty hashes (dan-course TJAs, never-played songs, fumen
    // charts whose hashes are computed lazily) concatenates to the SAME empty
    // single hash; binding it made single_hash_to_path[""] point at whichever
    // hashless chart was bound last (a Dan Dojo course in practice), and
    // load_from_song_list then resolved every empty-hash Recently Played /
    // Favorites row to that dan chart -- an unplayable SongBox whose
    // "difficulty" has no chart data, crashing Game2PScreen::init_tja.
...
std::optional<fs::path> ScoresManager::get_path_by_hash(const std::string& single_hash) {
    // ROUND 46: an empty hash identifies nothing -- see add_path_binding.
    // Guarding the lookup too keeps stale maps (built before that fix) and
    // empty-hash rows in existing song_list.txt files from resolving.
...
// ROUND 50 (r50-dani-visual-completion) — the 段位道場 best-result store.
// One row per (player, course title); `rank` = best odai_result + 1
// (DaniResultReward.ChangeNamePlateDani's normal_rank[dani] semantics), so an
// old database simply has no rows and every course reads back as "never
// passed". The table is created on first use, keeping the schema change
// backward-compatible (an old build ignores the extra table entirely).
static void ensure_dan_table(sqlite3* db) {
    sqlite3_exec(db,
        ...
        nullptr, nullptr, nullptr);
    // ROUND 57 — arrival_song_cnt. ALTER on an already-migrated DB fails with
    // "duplicate column name", which is the expected no-op (sqlite has no
    // ADD COLUMN IF NOT EXISTS); an r50-era DB gains the column with its 0
    // default, so nothing existing changes meaning.
    sqlite3_exec(db,
