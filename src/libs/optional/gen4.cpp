...
// DATATABLE_SEED and FUMEN_SEED are supplied at compile time (see .env and
// GEN4_ENABLED in CMakeLists.txt); they are not ours to publish in source.
...
std::vector<uint8_t> Library::load_chart(const std::string& id, int difficulty) const {
    ...
    // As in FumenParser::read_chart: a chart taken straight from the game
    // data is encrypted, one that has already been unpacked is not. Trying
    // the key tells the two apart; a plain file fails and is then used as it
    // is, rather than dropped as an empty chart.
