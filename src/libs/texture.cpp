...
// rapidjson's GetInt() ASSERTS when the stored number is not an integer.
// Release builds compile RAPIDJSON_ASSERT out, so GetInt() then reinterprets
// the stored double's bits as an int and returns garbage: a skin that wrote
// "x": 722.5 in a texture.json got its texture drawn thousands of pixels
// off-screen, silently, with no warning anywhere. Every skin-authored number
// is read through this instead.
//
// Integer literals take the identical GetInt() path and the callers keep their
// original static_cast<int>(value * scale) truncation, so every existing skin
// (PyTaikoGreen included) parses to bit-identical values. Fractional literals
// are honoured as written and truncated after scaling, instead of becoming
// noise.
...
void TextureWrapper::init(const fs::path& skin_path) {
    ...
    auto load_entry = [this](const std::string& name, const Value& v, float scale) {
        ...
        // ROUND 19: `outline` is deliberately NOT multiplied by `scale`.
        // It is in OutlinedText's outline-radius units, and OutlinedText's own
        // constructor already multiplies by global_tex.screen_scale
        // (src/libs/text.cpp:200). Scaling it here is exactly the double-scale
        // ROUND 16 found had been drawing every lumen-sourced outline 50 % too
        // thick across 19 call sites. Do not "fix" this by adding * scale.
    ...
    if (fs::exists(anim_file)) {
        ...
        if (anim_config.IsArray()) {
            for (SizeType i = 0; i < anim_config.Size(); i++) {
                ...
                // "waypoint" is a distance like total_distance (see MoveAnimation)
                if (anim.HasMember("waypoint") && !anim["waypoint"].IsObject()) {
                    if (anim["waypoint"].IsInt()) {
...
struct TexTimer {
    ...
    double ms() const { return std::chrono::duration<double, std::milli>(
        std::chrono::steady_clock::now() - t0).count(); }
};

// Process-wide cache of already-decoded texture objects, shared by every
// TextureWrapper instance (the engine's `tex` and `global_tex`, plus the Lua
// script manager's own wrapper). Without it a folder that both the engine and a
// Lua override draw from was decoded and uploaded to the GPU twice on every
// single screen change - on a large skin that was ~40% of the whole transition.
// weak_ptr, so the GPU memory is still released the moment the last wrapper
// holding the folder drops it.
...
// stb_image PNG decoding is pure CPU work and by far the largest part of a
// screen load, so it is spread over the machine's cores. Only the decode runs
// here; every GL call still happens on the thread that called load_folder.
...
// Frame directories are numbered 0.png, 1.png, ...
...
// texture.json keys the child skin actually ships a file for. The parent's copy
// of those is overwritten the moment the child pass runs, so decoding it is
// pure waste.
...
void TextureWrapper::load_folder(const std::string& screen_name, const std::string& subset) {
    ...
    auto load_from_path = [&](const fs::path& folder, float tex_scale,
        ...
        try {
            ...
            if (!files.empty())
                spdlog::debug("[perf]     {} : decode {:.1f} ms / gpu {:.1f} ms (mip {:.1f} ms) for {} files",
                             folder.string(), decode_ms, gpu_timer.ms(), mip_ms, files.size());
    ...
    // A child skin only ships the folders it overrides; a missing child
    // texture.json is the normal case, not an error.
    if (child_has_folder) {
...
void TextureWrapper::load_screen_textures(const std::string& screen_name) {
    ...
    spdlog::info("[perf]  textures for {}: {:.1f} ms", screen_name, screen_timer.ms());
...
void TextureWrapper::draw_texture(uint32_t id, const DrawTextureParams& params) {
    ...
    if (frame_tex) {
        if (params.blend.has_value()) {
            ...
            // Restore the mode the whole frame is drawn in (YataiDON.cpp opens
            // BLEND_CUSTOM_SEPARATE around every screen); EndBlendMode would
            // drop us back to plain BLEND_ALPHA instead.
